...
// ─── Stat Card ─────────────────────────────────────────────────────────────
const StatCard = ({ icon: Icon, label, value, sub, accent = "pink", dm, link }) => {
  const accents = {
  ...
  return (
    ...
    <div
      className={`relative rounded-2xl border p-4 sm:p-5 flex flex-col gap-3 overflow-hidden transition-all duration-300 hover:scale-[1.02] hover:shadow-xl ${ring[accent]
        } ${dm
    ...
    >
      ...
      <div className="flex items-start justify-between">
        ...
        {sub !== undefined && (
          <span
            className={`text-xs font-semibold px-2 py-1 rounded-lg ${sub > 0
      ...
      <div>
        <p
          className={`text-2xl sm:text-3xl font-extrabold tracking-tight ${dm ? "text-white" : "text-gray-900"
            }`}
        >
          {value}
        </p>
        <p
          className={`text-xs mt-0.5 font-medium ${dm ? "text-white/40" : "text-gray-500"
            }`}
        >
          {label}
        </p>
...
const SectionTitle = ({ title, dm }) => (
  <div className="flex items-center gap-3 mb-4">
    ...
    <h2
      className={`text-sm font-bold uppercase tracking-widest ${dm ? "text-white/60" : "text-gray-500"
        }`}
    >
      {title}
    </h2>
...
const CustomTooltip = ({ active, payload, label, dm }) => {
  ...
  return (
    <div
      className={`rounded-xl border px-4 py-3 text-xs shadow-xl ${dm
...
const UserRow = ({ user, dm, idx }) => (
  <div
    className={`flex items-center gap-3 py-3 border-b last:border-b-0 ${dm ? "border-white/5" : "border-gray-100"
      }`}
  >
    <span
      className={`text-xs w-5 text-center font-bold ${dm ? "text-white/20" : "text-gray-300"
        }`}
    >
      {idx + 1}
    </span>
    ...
    <span
      className={`text-xs px-2 py-0.5 rounded-full font-semibold capitalize ${user.gender === "male"
...
const CallRow = ({ call, dm }) => {
  ...
  return (
    ...
    >
      <div
        className={`p-1.5 rounded-lg ${isEnded ? "bg-emerald-500/10" : "bg-red-500/10"
          }`}
      >
        {call.callType === "video" ? (
          <FaVideo
            className={`text-xs ${isEnded ? "text-emerald-400" : "text-red-400"
              }`}
          />
        ) : (
          <FaPhoneAlt
      ...
      <div className="flex-1 min-w-0">
        <p
          className={`text-xs font-semibold truncate ${dm ? "text-white" : "text-gray-900"
            }`}
        >
      ...
      <span
        className={`text-xs px-2 py-0.5 rounded-full font-bold ${isEnded
...
const MiniStat = ({ label, value, dm }) => (
  <div
    className={`flex justify-between items-center py-2.5 border-b last:border-b-0 text-sm ${dm ? "border-white/5" : "border-gray-100"
      }`}
  >
...
const Panel = ({ title, children, dm, className = "" }) => (
  <div
    className={`rounded-2xl border p-4 sm:p-5 ${dm
  ...
  >
    {title && (
      <h3
        className={`text-sm font-bold mb-4 ${dm ? "text-white/60" : "text-gray-500"
...
const ProgressBar = ({ label, value, max, color = "pink", dm }) => {
  ...
  return (
    <div className="mb-3 last:mb-0">
      ...
      <div
        className={`h-2 rounded-full overflow-hidden ${dm ? "bg-white/5" : "bg-gray-100"
          }`}
      >
        <div
...
const Dashboard = ({ darkMode, collapsed }) => {
  ...
  if (loading) {
    return (
      <div
        className={`min-h-screen flex flex-col items-center justify-center gap-4 ${dm ? "bg-[#080808]" : "bg-gray-50"
          }`}
      >
        ...
        <p
          className={`text-sm font-medium animate-pulse ${dm ? "text-white/30" : "text-gray-400"
            }`}
        >
          Loading dashboard…
        </p>
  ...
  if (!data) {
    return (
      <div
        className={`min-h-screen flex items-center justify-center ${dm ? "bg-[#080808] text-white" : "bg-gray-50 text-gray-900"
          }`}
      >
  ...
  return (
    <div className={gridCls}>
      ...
      <div
        className={`rounded-2xl border overflow-hidden mb-8 ${dm ? "bg-[#0f0f0f] border-pink-500/20" : "bg-white border-pink-100"
          }`}
      >
        {/* Table header */}
        <div
          className={`grid grid-cols-4 sm:grid-cols-5 gap-3 px-4 sm:px-6 py-3 text-xs font-bold uppercase tracking-widest border-b ${dm
        ...
        {recentActivity.payments.map((p, i) =>
          p.userId?.name ? (
            <div
              key={p._id}
              className={`grid grid-cols-4 sm:grid-cols-5 gap-3 items-center px-4 sm:px-6 py-3.5 text-sm border-b last:border-b-0 transition-colors ${dm
            ...
            >
              ...
              <div className="col-span-2 flex items-center gap-2 min-w-0">
                ...
                <div className="min-w-0">
                  <p
                    className={`font-semibold truncate text-xs sm:text-sm ${dm ? "text-white" : "text-gray-900"
                      }`}
                  >
                    {p.userId?.name}
                  </p>
              ...
              <span className="hidden sm:block font-bold text-emerald-400">
                ₹{p.amount}
              </span>

              <span className="px-2 py-1 rounded-lg text-xs font-bold bg-amber-500/10 text-amber-400 w-fit">
                +{p.coins}
              </span>
            </div>
          ) : null
          ...
          <Panel dm={dm} className="mb-8">
            <div className="grid sm:grid-cols-2 gap-4">
              {data.settings.warningContents[0].description.map((d, i) => (
                <div
                  key={i}
                  className={`flex items-start gap-3 p-3 rounded-xl ${dm ? "bg-white/[0.03]" : "bg-pink-50/60"
                    }`}
                >
